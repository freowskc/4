...
var stamina_drain_rate = 4.0  # Stamina points drained per second
...
func _physics_process(delta):
	...
	if !is_resting:
		...
		if stamina <= 0:
			stamina = 0
			is_resting = true
			# Switch to idle animation when resting
...
func add_stamina(amount):
	...
	# If was resting and now has stamina, start moving again
	if is_resting and stamina > 0:
		is_resting = false
		# Switch back to walk animation when recovering from rest
		$AnimationPlayer.play("walk")
