...
func Enter():
	print("Pillow entered attack")
	start_attack_cooldown_timer()
	print("TEAM = ", pillow.team , "Player = ", pillow)
...
func start_attack_cooldown_timer():
	if attack_cooldown_timer == null:
		...
		attack_cooldown_timer.wait_time = 4
...
func on_all_enemies_die():
	print("on attack leaving to idle pillow")
	if attack_cooldown_timer != null:
		print("queue free timer node = ", attack_cooldown_timer)
		attack_cooldown_timer.queue_free()
	Leave(self, "idle")
...
func attack():
	...
	if enemy:
		...
		SignalEvents.unit_attack.emit(int(randf_range(0, 5)), enemy)
		attack_count += 1
